...
| `network_interfaces` (launch template, multi-NIC)   | `secondary_vnics`                                | one set of secondary VNICs attached to every node in the pool, including ones added later by scale-out. Requires `cni_type = "npn"`; the OCI API rejects it for Flannel Overlay clusters, enforced by a precondition |
