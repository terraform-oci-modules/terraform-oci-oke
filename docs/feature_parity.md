...
| `timeouts`                                           | fixed `delete = "75m"`                           | not user-configurable yet, see [class C](#gap-class-c-oke-capability-not-yet-exposed-real-backlog) |
...
| (no equivalent)                                      | `eviction_grace_duration`, `force_node_delete`   | OKE drain-on-delete controls, OKE-specific                   |
...
| `cluster_service_cidr`, `cluster_ip_family`      | via `cluster_all_attributes`                     | no dedicated outputs                                     |
...
| **`is_force_action_after_grace_duration`**      | `node_eviction_node_pool_settings` has two force flags; the module only sets the older `is_force_delete_after_grace_duration`. The newer flag is the resource-level equivalent of the CLI `--is-force-deletion-after-override-grace-duration` workaround documented in [testing.md](testing.md), so exposing it may reduce the stuck-teardown class of failures. | `force_delete` semantics on the ASG                      |
| **Per-pool `timeouts`**                         | The node pool and virtual node pool resources hardcode `delete = "75m"` (see [testing.md](testing.md) for why). `create`/`update` use provider defaults and none of the three is user-configurable, unlike the cluster's `timeouts`.                                                                          | `timeouts` on every EKS node group                       |
| **Dedicated `cluster_service_cidr` / `cluster_ip_family` outputs** | Both values are reachable only through `cluster_all_attributes`. EKS promotes them to first-class outputs because node groups consume them.                                                                                                       | `cluster_service_cidr`, `cluster_ip_family`              |
| **`network_launch_type` on node pools**         | Provider-supported, unexposed. Low value, listed for completeness.                                                                                                                                                                                                                                                   | none                                                     |
